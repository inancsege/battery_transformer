...
from utils_TRANSFORMER import truncated_normal_, InputEmbedding, MultiHeadSelfAttention, \
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/best_soh_tec_model_TRANSFORMER.pth")
...
def evaluate_model(model, test_loader):
    model.load_state_dict(torch.load("models/best_soh_tec_model_TRANSFORMER.pth"))
    model.eval()
