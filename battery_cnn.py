...
from utils_LSTM import SOHLSTM, SOHDataset, create_sequences
...
model = SOHLSTM(input_dim=NUM_FEATURES, hidden_dim=128, num_layers=2, dropout=0.1).to(device)
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=20):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_X, batch_y in train_loader:
            ...
            optimizer.zero_grad()
            outputs = model(batch_X).squeeze()  # LSTM output
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "models/best_soh_lstm_model.pth")
...
def evaluate_model(model, test_loader):
    model.load_state_dict(torch.load("models/best_soh_lstm_model.pth"))
    model.eval()
    ...
    with open('outputs/error_results_LSTM.txt', "w") as f:
        f.write(f"Test RMSE: {rmse:.4f}\nTest MAE: {mae:.4f}\nTest R²: {r2:.4f}")

time.sleep(2)

monitoring = True
monitor_thread = threading.Thread(target=monitor_gpu, args=('outputs/log_testing_LSTM.csv', 0.01), daemon=True)
monitor_thread.start()
